...
class Plotter:
    ...
    def plot_ver(self,ver):
        ...
        #plt.title(r'$\overline{n_{\mathrm{SW, 1AU}}} = 24.3$ cm$^{-3}$, $v_{\mathrm{SW}}=370$ km s$^{-1}$',fontsize=12)
        #plt.title('ER86 Maximal Neutrals')
        #plt.plot(x_mag, y_mag,color='blue',label='Magnetopause')
        #plt.plot(x_bow, y_bow,color='red',label='Bow Shock')
        ax.set_xlabel(r'$x$ ($R_{U}$)')
        ax.set_ylabel(r'$y$ ($R_{U}$)')
        ax.set_zlabel(r'$z$ ($R_{U}$)')
        #plt.legend()
        #ax.tick_params(axis='both', direction='out', top=True, bottom=True, left=True, right=True,labelsize=13)
        #plt.grid(False)
        #plt.tight_layout()
        #plt.savefig('MAY38-08-V3.png',dpi=1200)
