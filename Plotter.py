...
class Plotter:
    ...
    def plot_ver_combined(self,ver_slow,ver_fast):
        fig, axes = plt.subplots(1,2,figsize=(10,5))
        #plt.rcParams.update({'font.size': 13})
        axes[0].set_xlim([self.x_min, self.x_max])
        axes[0].set_ylim([self.y_min, self.y_max])
        contour_levels1 = np.linspace(intensity_values_slow.min(), intensity_values_slow.max(), 100)
        contour1 = axes[0].contourf(xbox, ybox, intensity_values_slow, levels=contour_levels1, cmap='YlOrRd')  
        plt.colorbar(contour1, label='Volumetric Emission (photon cm'r"$^{-3}$"' s'r"$^{-1}$"')', ax=axes[0])
        axes[0].plot(x_mag_slow, y_mag_slow, color='blue')
        axes[0].plot(x_bow_slow, y_bow_slow, color='red')
        axes[0].set_xlabel(r"$x$ ($R_{S}$)")
        axes[0].set_ylabel(r"$y$ ($R_{S}$)")
        #axes[0].set_title('Volumetric Emission')
        axes[0].set_title('(a)')
        axes[0].grid(False)
        axes[0].tick_params(axis='both', direction='out', top=True, bottom=True, left=True, right=True)
        contour_levels2 = np.linspace(intensity_values_fast.min(), intensity_values_fast.max(), 100)
        contour2 = axes[1].contourf(xbox,ybox,intensity_values_fast,levels=contour_levels2,cmap='YlOrRd')
        plt.colorbar(contour2, label=r'Volumetric Emission (photon cm$^{-3}$ s$^{-1}$)', ax=axes[1])
        axes[1].set_xlim([x_min,x_max])
        axes[1].set_ylim([y_min,y_max])
        axes[1].plot(x_mag_fast, y_mag_fast, color='blue')
        axes[1].plot(x_bow_fast, y_bow_fast, color='red')
        #axes[1].set_title('SMILE-like SXI Image')
        axes[1].set_title('(b)')
        axes[1].set_ylabel(r'$y$ ($R_{S}$)')
        axes[1].set_xlabel(r'$x$ ($R_{S}$)')
        axes[1].grid(False)
        #cbar = fig.colorbar(contour1, ax=axes.ravel().tolist())
        axes[1].tick_params(axis='both', direction='out', top=True, bottom=True, left=True, right=True)
        plt.tight_layout()
        #plt.savefig('Slow_Fast_VER_170.png',dpi=1200)
        ...
        ax[0].colorbar(xy_plane_slow,label=r'Emission Rate (photon cm$^{-3}$ s$^{-1}$)')
        ax[0].set_xlim(self.x_min,40)
        ax[0].set_ylim(self.y_min,self.y_max)
        ax[0].set_xlabel(r'$x$ ($R_{U}$)')
        ax[0].set_ylabel(r'$y$ ($R_{U}$)')
        xy_plane_fast = ax[0].contourf(self.X_grid[:,:,z_pos],self.Y_grid[:,:,z_pos],ver_fast[:,:,z_pos],cmap='YlOrRd')
        x_bow_f, y_bow_f = bs.surf_2D(self.x_max,self.x_min)
        x_mag_f, y_mag_f = mp.surf_2D(self.x_max,self.x_min)        
        ax[1].plot(x_bow_s,y_bow_s,color='red')
        ax[1].plot(x_mag_s,y_mag_s,color='blue')
        ax[1].colorbar(xy_plane_slow,label=r'Emission Rate (photon cm$^{-3}$ s$^{-1}$)')
        ...
        plt.gcf().text(0.05, 0.05, f"Max VER: {ver_max_3_s}", ha='left', fontsize=12)
        plt.gcf().text(0.05, 0.01, f"Mean VER: {ver_mean_3_s}", ha='left', fontsize=12)   
        #plt.savefig("VER_x-y_high_vsw_v3",dpi=1200)
